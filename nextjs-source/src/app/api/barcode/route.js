 ...
 *  - Open Library (ISBN-10 / ISBN-13, including trailing X)
 *  - Open Food Facts (food EAN)
 *  - Open Beauty Facts (cosmetics)
 */
export async function GET(req) {
  const raw = new URL(req.url).searchParams.get("code")?.trim() ?? "";
  const code = raw.toUpperCase().replace(/[^0-9X]/g, "");

  const isIsbn10 = /^\d{9}[\dX]$/.test(code);
  const isDigitsOnly = /^\d{8,14}$/.test(code);
  if (!isIsbn10 && !isDigitsOnly) {
    return NextResponse.json({ error: "invalid code" }, { status: 400 });
  }

  // Books (ISBN-10 / ISBN-13)
  if (isIsbn10 || code.length === 13) {
    try {
  ...
  // Food / grocery / generic consumer goods often indexed in OFF
  try {
