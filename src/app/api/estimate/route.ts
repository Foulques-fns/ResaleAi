...
import { CONDITIONS, type EstimatePayload, type Lang } from "@/lib/types";

export const dynamic = "force-dynamic";
export const maxDuration = 60;

type Body = EstimatePayload & {
  phase?: "identify" | "estimate";
  thumb?: string;
};
...
export async function POST(req: Request) {
  ...
  let hypotheses: Awaited<ReturnType<typeof identifyFromImages>> extends null
    ? never
    : NonNullable<Awaited<ReturnType<typeof identifyFromImages>>>["hypotheses"];
  hypotheses = [];
  ...
  try {
    ...
    return NextResponse.json(
      { error: "database_error", message: e instanceof Error ? e.message : String(e) },
      { status: 500 },
    );
  }
}
