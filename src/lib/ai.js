...
function normalizeResult(parsed) {
    const p = parsed;
    const hyps = Array.isArray(p.hypotheses)
        ? p.hypotheses.slice(0, 3).map((h) => ({
...
async function callAnthropic(key, images, sys) {
...
async function callGemini(key, images, sys) {
...
export async function identifyFromImages(images, lang, notes) {
