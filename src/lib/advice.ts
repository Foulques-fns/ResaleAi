import type { Condition, EstimateAdvice, Lang, PriceStats } from "./types";
import { CONDITION_LABELS } from "./pricing";

const CATEGORY_PLATFORMS: Record<string, { fr: string[]; en: string[] }> = {
...
const SEASON_BY_CATEGORY: Record<string, { months: number[]; fr: string; en: string }> = {
...
export function buildAdvice(input: {
  ...
  const platforms = (CATEGORY_PLATFORMS[cat][isEn ? "en" : "fr"] ?? CATEGORY_PLATFORMS.autre[isEn ? "en" : "fr"]).map(
    (name, i) => ({
      name,
      reason:
        i === 0
  ...
  if (cat === "mode") keywords.push(isEn ? "size + color + material" : "taille + couleur + matière");
  if (cat === "jeux-video" || cat === "high-tech") keywords.push(isEn ? "serial visible? / tested & working" : "testé & fonctionnel, numéro de série visible");
  if (cat === "collection" || cat === "art-antiquite") keywords.push(isEn ? "year / edition / signature" : "année / édition / signature");

  const photoTips: string[] = isEn
    ? [
