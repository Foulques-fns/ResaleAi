 ...
 */
export function marketMedian(listings, queryName) {
...
export function filterOutliers(listings) {
...
export function roundNice(n) {
    if (n <= 0)
        return 0;
    if (n < 10)
        return Math.round(n * 2) / 2; // .5 steps
    if (n < 30)
        return Math.round(n);
    if (n < 100)
...
export function computePrice(listings, condition, lang = "fr") {
    ...
    const stats = {
        ...
        newPriceHint: newItems.length > 0
    ...
    if (n > 0) {
        if (mid <= 0)
            mid = roundNice(stats.median * 0.85) || stats.median;
        if (low <= 0)
            low = roundNice(mid * 0.7);
        if (high < mid)
            high = mid;
        if (low > mid)
            low = roundNice(mid * 0.6);
    }
