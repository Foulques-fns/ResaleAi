...
export function isAccessoryNoise(title, queryTokens) {
    ...
    for (const noise of ACCESSORY_NOISE) {
        ...
        if (clean.length < 3)
            continue;
        if (clean.includes(" ")) {
            if (norm.includes(clean) && !queryTokens.some((t) => clean.includes(t)))
                return true;
        }
    ...
    for (const noise of DEFECT_NOISE) {
        ...
        if (hit && !queryTokens.some((t) => clean.includes(t)))
            return true;
    }
    return false;
}
...
export function buildQueryTokens(parts) {
    ...
    const seen = new Set([...brandTokens, ...modelTokens]);
...
function tokenMatch(haystack, token) {
    ...
    if (token.length >= 5) {
        for (const w of haystack.split(" ")) {
            if (w.length >= 5 && (token.startsWith(w) || w.startsWith(token)))
                return true;
        }
    }
    return false;
 ...
 */
function modelTokenMatch(haystack, modelToken, queryTokens) {
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        const exact = modelToken.length <= 4
        ...
        const windowWords = [words[i - 1], words[i + 1]].filter(Boolean);
        if (others.some((t) => windowWords.some((ww) => tokenMatch(ww, t))))
            return true;
    }
    return false;
}
function scoreOne(l, tokens, brandTokens, modelTokens, requireBrand, requireModel, coverage) {
    if (tokens.length === 0)
        return { ...l, score: 1 };
    ...
    for (const t of tokens)
        if (tokenMatch(hay, t))
            matched++;
    const brandOk = brandTokens.length === 0 ||
    ...
    if (requireBrand && brandTokens.length > 0 && !brandOk)
        return null;
    if (requireModel && modelTokens.length > 0 && !modelOk)
        return null;
 ...
 */
export function filterRelevant(listings, parts, minKeep = 3) {
    ...
    for (const l of listings) {
        if (isAccessoryNoise(l.title, tokens))
            noiseDropped.push(l);
        else
            clean.push(l);
    }
    ...
    for (let level = 0; level < levels.length; level++) {
        if (level === 0 && brandTokens.length === 0 && modelTokens.length === 0)
            continue;
        if (level === 1 && brandTokens.length === 0)
            continue;
 ...
 */
export function isNewListing(l) {
    if (l.source === "dealabs")
        return true; // deals = retail promos
    return NEW_PATTERNS.test(`${l.conditionText ?? ""} ${l.title}`);
}
export function usedPool(listings) {
    const used = [];
    const newItems = [];
    for (const l of listings) {
        if (isNewListing(l))
...
export function mapConditionText(conditionText) {
    if (!conditionText)
        return null;
    const c = normalizeText(conditionText);
    if (/neuf|new/.test(c))
        return "new";
    if (/tres bon|very good/.test(c))
        return "like_new";
    if (/bon etat|^good$/.test(c))
        return "good";
    if (/satisfaisant|correct|satisfactory|fair/.test(c))
        return "fair";
    if (/use|mauvais|worn|poor/.test(c))
        return "poor";
    return null;
}
 ...
 */
export function conditionProximity(pool, declared) {
    const idx = CONDITION_ORDER.indexOf(declared);
    // only worth filtering when the pool is comfortably large
    if (idx < 0 || pool.length < 10)
        return pool;
