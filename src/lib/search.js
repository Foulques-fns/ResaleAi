...
function parsePrice(raw) {
    ...
    if (!Number.isFinite(v) || v <= 0 || v > 500000)
        return null;
...
/* ---------------------------------- Vinted (SSR catalog) ---------------------------------- */
async function vintedProvider(query, lang) {
    const host = lang === "en" ? "www.vinted.co.uk" : "www.vinted.fr";
    const start = Date.now();
    try {
        const html = await fetchText(`https://${host}/catalog?search_text=${encodeURIComponent(query)}&order=newest_first`);
        ...
        while ((m = tileRe.exec(html)) && listings.length < 20) {
            ...
            for (const p of parts) {
                if (p.startsWith(`${label}:`))
                    brand = p.slice(label.length + 1).trim();
                if (p.startsWith(`${condLabel}:`))
                    conditionText = p.slice(condLabel.length + 1).trim();
            }
            listings.push({
                title,
                brand,
...
async function dealabsProvider(query, _lang) {
    const start = Date.now();
    try {
        const html = await fetchText(`https://www.dealabs.com/search?q=${encodeURIComponent(query)}`);
        const listings = [];
        const linkRe = /<a[^>]*class="[^"]*thread-link[^"]*"[^>]*title="([^"]{4,220})"[^>]*href="(https:\/\/www\.dealabs\.com\/[^"]+)"[^>]*>/g;
        ...
        for (let i = 0; i < matches.length && listings.length < 15; i++) {
            ...
            if (/\/\s?mois|par mois|mensualit/i.test(window.slice(0, 2000)))
                continue;
...
async function ebayProvider(query, lang) {
    const start = Date.now();
    ...
    try {
        ...
        while ((m = itemRe.exec(html)) && listings.length < 20) {
            ...
            if (/Shop on eBay|Results matching/i.test(title))
                continue;
...
async function leboncoinProvider(query, _lang) {
    const start = Date.now();
    try {
        const html = await fetchText(`https://www.leboncoin.fr/recherche?text=${encodeURIComponent(query)}&sort=time`);
        const listings = [];
...
async function bingProvider(query, lang) {
    const start = Date.now();
    try {
        const q = lang === "en"
            ? `${query} used price sold`
            : `${query} occasion prix € vendu`;
        const html = await fetchText(`https://www.bing.com/search?q=${encodeURIComponent(q)}&setlang=${lang === "en" ? "en-GB" : "fr-FR"}&cc=${lang === "en" ? "GB" : "FR"}`);
        const listings = [];
...
async function duckduckgoProvider(query, lang) {
    const start = Date.now();
    try {
...
export async function liveSearchComparables(query, lang) {
