...
(() => {
  ...
  function openBarcodeScanner() {
    ...
    modal.innerHTML = `
      ...
      <div class="camera-ui">
        ...
        <div class="camera-bottom">
          ...
          <button class="camera-btn-round" data-code="manual">${PS.ICONS.barcode}</button>
          <span style="width:3rem"></span>
        </div>
      </div>`;
    document.body.appendChild(modal);
    ...
    function cleanup(detectedCode) {
      ...
      modal.remove();
      document.removeEventListener("visibilitychange", onVis);
      if (detectedCode) applyBarcode(detectedCode);
    }
    ...
    modal.addEventListener("click", async (e) => {
      ...
      if (act === "manual") { cleanup(null); setTimeout(() => document.getElementById("ean")?.focus(), 80); }
      if (act === "flip") {
        const currentFacing = barcodeVideoTrack?.getSettings?.()?.facingMode || "environment";
        const newFacing = currentFacing === "user" ? "environment" : "user";
        stopBarcodeCamera();
        try {
          const s = await navigator.mediaDevices.getUserMedia({ audio: false, video: { facingMode: { ideal: newFacing }, width: { ideal: 1920 }, height: { ideal: 1080 } } });
          barcodeStream = s;
          barcodeVideoTrack = s.getVideoTracks()[0] || null;
          video.srcObject = s;
          video.play();
  ...
  function applyBarcode(raw) {
    ...
    if (eanInput) {
      ...
      state.eanOk = true;
      if (eanMsg) { eanMsg.textContent = state.eanMsg; eanMsg.className = "xsmall mt-1 mint-text"; }
      PS.toast(`📷 ${code}`);
    }
    doLookup(code);
  }
  ...
  async function doLookup(code) {
    try {
      const res = await apiFetch(`/api/barcode?code=${encodeURIComponent(code)}`, { cache: "no-store", timeout: 8000 });
    ...
    const eanMsg = document.getElementById("eanMsg");
    if (eanMsg) { eanMsg.textContent = state.eanMsg; eanMsg.className = `xsmall mt-1 ${state.eanOk ? "mint-text" : "muted"}`; }
    const fName = document.getElementById("fName");
    if (fName && state.name) fName.value = state.name;
  }
  ...
  async function onFiles(files) {
    for (const file of [...files].slice(0, MAX_PHOTOS)) {
      try {
        ...
        addPhoto(canvas.toDataURL("image/jpeg", 0.90));
      } catch {}
    }
  }

  const nativeInput = document.createElement("input");
  nativeInput.type = "file"; nativeInput.accept = "image/*"; nativeInput.capture = "environment"; nativeInput.multiple = true; nativeInput.className = "hidden";
  nativeInput.onchange = (e) => { onFiles(e.target.files); nativeInput.value = ""; };
  const galInput = document.createElement("input");
  galInput.type = "file"; galInput.accept = "image/*"; galInput.multiple = true; galInput.className = "hidden";
  ...
  function renderCapture() {
    ...
    main.innerHTML = `
      <div class="stack">
        ...
        <section class="reveal reveal-1"><div class="grid-photos" id="photoGrid"></div></section>
        ...
        <section class="reveal reveal-4">
          ...
          <div style="display:flex;gap:0.5rem;flex-wrap:wrap;align-items:center">
            <div class="input-icon" style="flex:1;min-width:9rem">
              ${PS.ICONS.barcode}
              <input id="ean" class="input" inputmode="text" maxlength="14" autocapitalize="characters" placeholder="${T2.scan.barcodePh}" value="${state.barcode}"/>
            </div>
    ...
    state.photos.forEach((p, i) => {
      ...
      cell.innerHTML = `<img src="${p.edited}" alt=""/>${i === 0 ? '<span class="photo-first">1</span>' : ""}
        <div class="photo-actions">
          <button data-act="crop">${T2.scan.crop}</button>
          <button class="danger" data-act="del">${PS.ICONS.trash.replace('width="20"','width="14"').replace('height="20"','width="14"')}</button>
        </div>`;
    ...
    if (state.photos.length < MAX_PHOTOS) {
      ...
      camBtn.className = "photo-add photo-add--cam"; camBtn.innerHTML = `<span>${PS.ICONS.camera}<br/>${T2.scan.cam}</span>`;
      ...
      if (state.photos.length === 0) {
        ...
        galBtn.className = "photo-add photo-add--gal"; galBtn.innerHTML = `<span>${PS.ICONS.image}<br/>${T2.scan.gal}</span>`;
    ...
    PS.CONDITIONS.forEach((c) => {
      const b = document.createElement("button"); b.className = "cond-btn" + (state.condition === c ? " active" : "");
      b.textContent = T2.res.cond[c]; b.onclick = () => { state.condition = c; render(); };
      document.getElementById("condRow").appendChild(b);
    });

    const notesEl = document.getElementById("notes"); notesEl.value = state.notes; notesEl.oninput = () => (state.notes = notesEl.value);
    const ean = document.getElementById("ean"); ean.value = state.barcode;
  ...
  async function startIdentify() {
    state.error = null;
    if (state.photos.length === 0) { state.ai = { aiAvailable: false, hypotheses: [], suspicious: null, category: null }; state.step = "confirm"; render(); return; }
    showProgress(0);
    try {
      const res = await apiFetch("/api/estimate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phase: "identify", images: state.photos.map((p) => p.edited), condition: state.condition, notes: state.notes, lang: LANG }) });
      ...
      hideProgress(); state.step = "confirm"; render();
    } catch (e) {
      hideProgress();
      state.error = (LANG === "fr" ? "L'analyse a échoué (serveur injoignable ou erreur). Réessayez." : "Analysis failed (server unreachable or error). Please try again.");
  ...
  function renderConfirm() {
    const T2 = T(); const ai = state.ai || { hypotheses: [], suspicious: null, aiAvailable: false };
    main.innerHTML = `
      <div class="stack">
        <div class="reveal"><h1 class="h-page">${ai.hypotheses.length > 1 ? T2.scan.hypTitle : ai.hypotheses.length === 1 ? T2.scan.identified : T2.scan.title}</h1><p class="small muted mt-1">${ai.hypotheses.length > 1 ? T2.scan.hypHint : ai.aiAvailable ? T2.res.guided : T2.scan.aiMissing}</p></div>
        ${ai.suspicious ? `<div class="danger-box reveal">${ai.suspicious}</div>` : ""}
        <div id="hyps" class="stack-sm reveal reveal-1"></div>
        <div class="card card--pad stack-sm reveal reveal-2"><p class="field-label" style="margin:0">${T2.scan.manual}</p><input id="fName" class="input" placeholder="${T2.scan.namePh}"/><div class="grid-2"><input id="fBrand" class="input" placeholder="${T2.scan.brand}"/><input id="fModel" class="input" placeholder="${T2.scan.model}"/></div></div>
        <label style="display:flex;gap:0.5rem;align-items:flex-start;font-size:0.75rem" class="muted reveal reveal-3"><input id="lotChk" type="checkbox" ${state.lotMode ? "checked" : ""} style="width:1rem;height:1rem;accent-color:var(--acid);margin-top:0.1rem"/><span><strong style="color:var(--bone)">${T2.scan.lot}</strong> — ${T2.scan.lotHint}</span></label>
    ...
    (ai.hypotheses || []).forEach((h, i) => {
      const b = document.createElement("button"); b.className = "card hyp-card" + (i === state.selectedHyp ? " selected" : "");
      b.innerHTML = `<div style="display:flex;justify-content:space-between;gap:0.5rem;align-items:center"><p style="font-size:0.875rem;font-weight:700"></p><span class="chip ${h.confidence >= 0.75 ? "chip--mint" : h.confidence >= 0.5 ? "chip--warn" : "chip--danger"}">${Math.round((h.confidence || 0) * 100)}%</span></div><p class="xsmall muted mt-1">${[h.brand, h.model, h.category].filter(Boolean).join(" · ")}</p>${h.rationale ? `<p class="xsmall muted mt-1" style="font-style:italic">${h.rationale}</p>` : ""}`;
    ...
    const fName = document.getElementById("fName"); const fBrand = document.getElementById("fBrand"); const fModel = document.getElementById("fModel");
    fName.value = state.name; fBrand.value = state.brand; fModel.value = state.model;
    fName.oninput = () => (state.name = fName.value); fBrand.oninput = () => (state.brand = fBrand.value); fModel.oninput = () => (state.model = fModel.value);
  ...
  function showProgress(step) {
    ...
    main.innerHTML = `<div class="loading-center" style="flex-direction:column;gap:1.5rem;min-height:70dvh"><div class="scan-frame progress-frame">${state.photos[0] ? `<img src="${state.photos[0].edited}" alt=""/>` : `<span style="display:grid;place-items:center;height:100%;color:var(--acid)">${PS.ICONS.search.replace('width="20"','width="40"')}</span>`}<span class="scan-laser"></span></div><div class="progress-steps">${steps.map((s, i) => `<div class="progress-step ${i < step ? "done" : i === step ? "active" : ""}"><span class="ico">${i < step ? PS.ICONS.check.replace('width="20"','width="16"') : i === step ? PS.spinnerHTML() : '<span style="display:inline-block;width:1.25rem;height:1.25rem;border:1px solid var(--line);border-radius:50%"></span>'}</span>${s}</div>`).join("")}</div></div>`;
  }
  function advanceProgress(i) {
    document.querySelectorAll(".progress-step").forEach((el, idx) => {
      el.classList.remove("active", "done"); const ico = el.querySelector(".ico");
      if (idx < i) { el.classList.add("done"); ico.innerHTML = PS.ICONS.check.replace('width="20"','width="16"'); }
  ...
  async function runEstimate() {
    ...
    showProgress(0);
    let p = 1; progressTimer = setInterval(() => advanceProgress(Math.min(p++, 3)), 2300);
    try {
      const thumb = state.photos[0] ? await PS.cropImage(state.photos[0].edited, { zoom: 1, offsetX: 0, offsetY: 0, outSize: 160 }) : undefined;
      const res = await apiFetch("/api/estimate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phase: "estimate", images: state.photos.map((ph) => ph.edited), thumb, condition: state.condition, notes: state.notes, lang: LANG, barcode: state.barcode || undefined, confirmedLabel: state.name.trim(), confirmedBrand: state.brand.trim(), confirmedModel: state.model.trim() }) });
      if (!res.ok) throw new Error("estimate failed");
      const result = await res.json();
      PS.cacheEst(result.id, result); if (state.lotMode) PS.addLot(result.id);
      location.href = `/estimation.html?id=${result.id}${state.lotMode ? "&lot=1" : ""}`;
    } catch (e) {
      hideProgress();
      state.error = (LANG === "fr" ? "L'estimation a échoué. Réessayez." : "The estimate failed. Please try again.");
      state.step = "confirm"; render();
    }
  }
  render();
